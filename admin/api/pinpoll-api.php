    ...
    public function pinpoll_signin( $body, $redirect ) {
      ...
      if( !is_wp_error( $response ) ) {
        ...
        if( $response['response']['code'] === 200 ) {
          ...
          printf( '<div class="wrap"><div class="error notice notice-error is-dismissible"> <p> %s </p> ' . $error . ' </div></div>', esc_html__( $this->texts['error'], 'pinpoll' )  );
        }
      } else {
    ...
    function call_with_refresh_token_and_signin($callback) {
      ...
      if(!empty($response) && ($response['response']['code'] != 200)) {
        ...
        if( (isset($refreshResponse) && !$refreshResponse['success']) || (!empty($responseBody) && array_key_exists('error', $responseBody) && $responseBody['error'] === 'token_invalid')) {
          if ($this->pinpoll_refresh_sign_in()){
            return $callback();
          } else {
            //refresh signin failed
            printf('<script>window.location.href="admin.php?page=settings&refreshError=true"</script>');
          }
        } else {
          // unrecoverable error
          printf( '<div class="wrap"><div class="error notice notice-error is-dismissible"> <p> %s </p><p><b>Details:</b></br> %s </p> </div></div>', esc_html__($this->texts['errorfatal'], 'pinpoll'), implode("</br>",$responseBody) );
          die;
        }
      }
      return $response;
