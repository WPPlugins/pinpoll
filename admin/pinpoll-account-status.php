...
require_once( 'resources/gtm.php' );
...
class PinpollAccount
{
  ...
  //default construct
	...
	function pinpoll_user_in_wp() {
		...
		if (isset( $_GET['refreshError'] ) && $_GET['refreshError'] == 'true'){
			if (empty(get_option('pinpoll_account')['appkey'])){
   ...
   */
	function pinpoll_user_exists() {
		...
		if( $responseData['response']['code'] !== 200 && $responseData['response']['code'] != 422 ) {
			printf( '<div class="wrap"><div class="error"> <p> %s ' . $responseData['response']['message'] . '</p> </div></div>', esc_html__( $this->texts['error'], 'pinpoll' ) );
		} else {
	...
	function pinpoll_register_user() {
		...
		if( empty( $error ) && !empty( $responseJWT ) ) {
			...
			printf( '<div class="wrap"><div class="error"> <p> %s ' . $error . ' </p> </div></div>', esc_html__( $this->texts['error'], 'pinpoll' )  );
		}
	}
